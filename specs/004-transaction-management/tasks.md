...
- [X] T001 Verify branch `004-transaction-management` in `.`,
  `../zunera-backend`, and `../zunera-frontend` before editing application
  files.
- [X] T002 [P] Create transaction domain folders in
...
- [X] T003 [P] Create transaction feature folders in
...
- [X] T007 [P] Create lowercased, accent-stripped description-plus-notes
...
- [X] T008 [P] Create typed transaction state exception with
...
- [X] T009 Create the transactions and transaction-mutation-request tables:
...
- [X] T010 Create the guarded `Transaction` model with casts, `user`,
...
- [X] T011 [P] Create `TransactionFactory` with income, expense, pending,
...
- [X] T012 [P] Create `CreateTransactionData`, `UpdateTransactionData`, and
  `TransactionFilterData` in
  `../zunera-backend/app/Data/Transactions/`.
- [X] T013 [P] Create `ListTransactionsRequest`, `StoreTransactionRequest`,
...
- [X] T014 Implement `TransactionBalanceReconciler` that applies the previous
...
- [X] T015 Create `TransactionResource` exposing owner-safe fields plus embedded
...
- [X] T019 [P] [US1] Feature test for income and expense creation, note
...
- [X] T020 [P] [US1] Unit test for amount bounds, future-dated pending default,
...
- [X] T021 [US1] Implement `TransactionService::create` with owner-scoped
...
- [X] T022 [US1] Implement `StoreTransactionRequest` validation rules, DTO
...
- [X] T023 [US1] Wire `TransactionController::store` with the created-resource
  response in
  `../zunera-backend/app/Http/Controllers/Api/V1/TransactionController.php`.
- [X] T024 [US1] Run `php artisan test --compact tests/Feature/Transactions
  tests/Unit/Transactions` and `vendor/bin/pint --dirty --format agent` in
  `../zunera-backend`.

#### Frontend (after Backend)

- [X] T025 [P] [US1] Implement `createTransaction`, `listTransactions`, and
...
- [X] T026 [US1] Implement setup-style `transactionStore` with list state,
  active transaction, and create action in
  `../zunera-frontend/src/stores/transactions/transactionStore.js`.
- [X] T027 [US1] Build the record form dialog with Element Plus controls,
...
- [X] T028 [US1] Build the basic newest-first history list showing description,
...
- [X] T029 [US1] Add `/app/transactions` route, navigation entry, and i18n keys
  in `../zunera-frontend/src/router/index.js`,
  `../zunera-frontend/src/composables/`, and
  `../zunera-frontend/src/i18n/`.
- [X] T030 [P] [US1] Add service, store, and formatter unit tests covering
...
- [X] T031 [US1] Add Playwright journey that records an income and an expense
  transaction and verifies them in history in
  `../zunera-frontend/e2e/transactions.spec.js`.
- [X] T032 [US1] Run `npm run test:unit -- --run`, `npm run build`, and
...
- [X] T033 [P] [US2] Unit test the reconciliation matrix (income, expense,
...
- [X] T034 [P] [US2] Feature test balance effects for creation and status
...
- [X] T036 [US2] Integrate reconciliation for create and status changes, and
  ...
  `../zunera-backend/app/Services/Transactions/TransactionService.php`.
- [X] T037 [US2] Verify account list, show, and summary responses reflect
...
- [X] T038 [US2] Refresh the financial accounts store after every transaction
...
- [X] T039 [P] [US2] Add unit tests proving the accounts store refreshes after
  transaction mutations in
  `../zunera-frontend/src/stores/transactions/__tests__/`.
- [X] T040 [US2] Extend `../zunera-frontend/e2e/transactions.spec.js` to assert
  account balances after create, edit, and removal.
- [X] T041 [US2] Run focused backend and frontend tests for balance behavior.
...
- [X] T042 [P] [US3] Feature test for history ordering, entry fields, pagination
...
- [X] T043 [P] [US3] Feature test for transaction details, privacy-safe 404 for
...
- [X] T044 [P] [US3] Feature test for history scale: seed 5,000 transactions for
...
- [X] T045 [US3] Implement paginated `TransactionService::list`, `findOwned`,
  and the matching-count metadata in
  `../zunera-backend/app/Services/Transactions/TransactionService.php`.
- [X] T046 [US3] Implement `ListTransactionsRequest` pagination validation with
  a default and maximum page size of 50 in
  `../zunera-backend/app/Http/Requests/Transactions/ListTransactionsRequest.php`.
- [X] T047 [US3] Wire `TransactionController::index` and `show` in
  `../zunera-backend/app/Http/Controllers/Api/V1/TransactionController.php`
  and run the focused history and scale tests.

#### Frontend (after Backend)

- [X] T048 [US3] Add progressive loading, matching-count display, loading,
  error, and empty states to
  `../zunera-frontend/src/views/transactions/TransactionsListView.vue`.
- [X] T049 [US3] Build the transaction details drawer with all recorded fields,
  status, note, and archived association labels in
  `../zunera-frontend/src/components/transactions/TransactionDetailDrawer.vue`.
- [X] T050 [P] [US3] Add view and component unit tests for list states, ordering
...
- [X] T051 [US3] Extend `../zunera-frontend/e2e/transactions.spec.js` with
...
- [X] T052 [P] [US4] Feature test for archived associations: transactions stay
...
- [X] T053 [P] [US4] Feature test for each editable field, type/category
...
- [X] T054 [P] [US4] Feature test for removal, removed view, restore, repeated
...
- [X] T055 [US4] Implement `UpdateTransactionRequest` rules including the
...
- [X] T056 [US4] Implement `TransactionService::update` with change detection,
  ...
  `../zunera-backend/app/Services/Transactions/TransactionService.php`.
- [X] T057 [US4] Implement `TransactionService::remove` and `restore` with state
  conflicts, idempotency replay/conflict behavior, the removed view scope, and
  exactly-once reconciliation in
  `../zunera-backend/app/Services/Transactions/TransactionService.php`.
- [X] T058 [US4] Wire `TransactionController::update`, `remove`, and `restore`
  with typed conflict responses in
  `../zunera-backend/app/Http/Controllers/Api/V1/TransactionController.php`,
  then run the focused update and lifecycle tests.

- [X] T059 [US2] Feature test balance effects for type change, account move,
...
- [X] T061 [US4] Add `updateTransaction`, `removeTransaction`, and
  ...
  actions in
  `../zunera-frontend/src/stores/transactions/transactionStore.js`.
- [X] T062 [US4] Build the removed-transactions view with Restore actions,
  ...
  `../zunera-frontend/src/i18n/`.
- [X] T063 [P] [US4] Add unit tests for update, remove, restore, typed date notice,
...
- [X] T064 [US4] Extend `../zunera-frontend/e2e/transactions.spec.js` with edit,
...
- [X] T065 [P] [US5] Feature test for each filter, ISO and Brazilian date input,
...
- [X] T066 [P] [US5] Feature test for description and note search, case and
...
- [X] T067 [US5] Implement filter and search parsing into `TransactionFilterData`
  in
  `../zunera-backend/app/Http/Requests/Transactions/ListTransactionsRequest.php`
  and `../zunera-backend/app/Data/Transactions/TransactionFilterData.php`.
- [X] T068 [US5] Implement filter, normalized `search_text`, and removed-view
...
- [X] T069 [US5] Build the filter bar with date range, type, account, category,
  status, and text search controls in
  `../zunera-frontend/src/components/transactions/TransactionFilterBar.vue`.
- [X] T070 [US5] Keep filters in transaction store state and in the route query
  ...
  `../zunera-frontend/src/views/transactions/TransactionsListView.vue`.
- [X] T071 [P] [US5] Add unit tests for filter state, query-string sync, and
  clearing filters in
  `../zunera-frontend/src/stores/transactions/__tests__/` and
  `../zunera-frontend/src/components/transactions/__tests__/`.
- [X] T072 [US5] Extend `../zunera-frontend/e2e/transactions.spec.js` with
...
- [X] T073 [P] Lint the contract with
  `npx @redocly/cli lint specs/004-transaction-management/contracts/transactions-api.yaml`
...
- [X] T075 [P] Run `php artisan test --compact` and
  `vendor/bin/pint --format agent` in `../zunera-backend`.
- [X] T076 [P] Run `npm run test:unit -- --run` and `npm run build` in
  `../zunera-frontend`.
