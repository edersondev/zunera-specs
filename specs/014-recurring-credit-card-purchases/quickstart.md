...
## Acceptance execution record (2026-09-25)

Automated fixture walkthroughs exercise the paths above. The backend suite covers source-linked automatic purchases, retained confirmation choices after failure, competing overlapping actions, explicit over-limit approval and stale-credit rejection, pause/end/edit catch-up, archive repair, missed-date partial progress, backdated paid statements, corrections/refunds, and account/Budget/Dashboard/history projections. The focused browser journeys cover expected → open purchase → closed expense → paid statement, over-limit approval, confirmation/dismissal, and responsive recurrence and dashboard views.

| Criterion | Executed evidence | Result / remaining gap |
| --- | --- | --- |
| SC-001 | Browser creation journey and form/readback assertions in `e2e/recurring-transactions.spec.js` | Automated journey passes. Timed uncoached study with 20 representative owners remains to be recruited and run. |
| SC-002 | 100 month-end/leap-year automatic generation cases in `RecurringCardAutomaticGenerationTest::monthlyBoundaryCases`, plus pause/resume, inclusive end-date, and downtime tests | 100 boundary fixtures pass; named lifecycle boundaries pass in separate tests. |
| SC-003 | 100 genuinely overlapping generation attempts, 100 overlapping automatic decisions, 100 overlapping distinct confirmation choices, and 100 repeated confirmations in the recurring feature tests and `OverlappingCardAttempts` harness | Exactly one purchase/source date; overlap uses workers against a shared isolated database. |
| SC-004 | 100 dated billing fixtures in `RecurringCardBillingParityTest::billingBoundaryCases`, comparing recurring and manual single-payment purchases on cards with the same closing/due days and credit limit | 100 cases, 1,400 assertions pass: closing date, due date, statement status, recognition date, installment amount, used credit, and available credit agree. |
| SC-005 | 100 old-versus-new amount edit cases in `RecurringCardLifecycleTest::futureOnlyEditCases`; separate archive, failure, pause/end, correction, refund, and reporting tests | 100 edit fixtures pass; other named exception paths pass in separate tests. |
| SC-006 | Browser labels distinguish expected, recorded, externally unverified, and installment states in recurrence, Dashboard, and card activity | Automated copy and state assertions pass. Timed classification study with 20 representative owners remains to be recruited and run. |
| SC-007 | `e2e/recurring-transactions-performance.spec.js`: 1,000 owned account/card rules and representative occurrences; 2 warmups, then 20 list and 20 detail timings per viewport | Chromium desktop 1280×800: list p95 707 ms, detail p95 251 ms. Narrow mobile 320×640: list p95 693 ms, detail p95 190 ms. All primary controls usable. Local Playwright browser, default CPU, local preview server, mocked API with 25 ms list latency and no network throttling. Backend 1,000-card-rule query-count test asserts fewer than 12 list queries and fewer than 25 occurrence-list queries. |

The SC-001 and SC-006 owner study is the remaining acceptance gap. For each of at least 20 owners, record device, theme, configuration time and correct readback, then classification time and answer. Thresholds are 19/20 under three minutes with correct rule details and 18/20 under 30 seconds with correct classification. Do not collect card credentials. The automated browser timings above describe a local mocked network; repeat SC-007 in a representative deployed environment if production device/network conditions differ materially.
