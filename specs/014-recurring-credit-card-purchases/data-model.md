...
| Confirmation action claim and choice version | Opaque active-attempt identity, bounded recovery time, and revision of the retained owner choices. Only the owning attempt may change choices or record a purchase for that revision. A stale attempt can be released only after the source-linked purchase is checked; these fields do not represent a new financial state. |
...
The card processor must commit one date at a time, rather than keep the existing whole-rule transaction around a backlog. A represented outcome and cursor progress commit together for each date; a purchase and `recorded` transition also commit together. A recoverable purchase error rolls back only that date's financial mutation, then stores a privacy-safe `failed` occurrence in a separate transaction and continues with later dates. If even failure identity cannot be stored, cursor progress stops before that date, the rule's remaining dates are deferred, and earlier date commits remain valid. A failed state is used only when it can be stored safely outside the failed purchase transaction.

For confirmation, acquire a durable claim and persist validated owner choices as one workflow step before the purchase transaction. While the claim is active, a different attempt cannot overwrite its choices or dismiss the item. The purchase transaction locks the occurrence, verifies claim/choice version, uses those same values, and commits purchase plus `recorded` transition atomically. After failure, release the claim while retaining its choices in `failed`; after an interrupted attempt, first check whether a source-linked purchase committed before a bounded stale-claim recovery. Retry rechecks current card/category and credit eligibility. Uniqueness protects the source, while claim/version checks protect the consistency of occurrence audit values and purchase fields.

Before an owner-initiated card rule edit, pause, or end, process or durably represent all eligible active dates already due through the current São Paulo business date with the pre-change rule values. Recheck the rule and any remaining due dates under a lock before mutation. If a due date cannot be represented, reject that rule mutation without reversing earlier independent date commits. A represented `failed` or `awaiting_over_limit` occurrence keeps its original snapshot after the rule changes. Automatic pausing for an unavailable card or category follows existing association availability semantics.
