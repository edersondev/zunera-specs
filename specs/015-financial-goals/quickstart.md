...
3. Link a second goal to the account and use overlapping requests to allocate its remaining capacity. Only one request may claim capacity beyond the shared limit. Record a real expense afterward that lowers actual balance below total allocations; verify exact shortfall warning, the overview's linked-shortfall goal count, no auto-withdrawal, and no new linked addition until capacity returns.
...
6. Set a future date and verify rounded monthly guidance over inclusive remaining calendar months. Verify due-today/overdue and reached-target cases show no suggestion; an overdue underfunded active goal increments the overview attention count, while a completed goal does not. No recurring transaction is created.
7. Archive a linked account and confirm the goal remains readable, the overview's inactive-or-unavailable linked-goal count updates, additions stop, and withdrawal/reassociation remain possible. Compare Financial Accounts, Transfers, Transactions, Credit Cards, Budget, financial history, and Dashboard before and after goal-only actions for no duplicate financial recognition.
