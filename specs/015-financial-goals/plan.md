...
5. **Backend reads and integration**: Derive current allocation from accepted goal activities, progress, rounded monthly guidance, active-only overview totals, and a maximum three-goal Dashboard projection. Keep goal activity outside Financial History, Budget calculation, account summaries, Dashboard cash-flow totals, card obligations, and recurrence outputs.
